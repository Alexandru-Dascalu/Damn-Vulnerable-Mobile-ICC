...
class NewsService : Service() {
    ...
    override fun onCreate() {
        ...
        securityLevel = loadSecuritySettingsFromFile()
    }
    ...
    private fun sendResultBroadcast(articles: ArrayList<ArticleDto>) {
        val jsonArticles = Gson().toJson(articles)
        sendResultBroadcast(jsonArticles)
    }

    private fun sendResultBroadcast(articles: String) {
    ...
    private fun loadSecuritySettingsFromFile() : String {
        val storageState = Environment.getExternalStorageState()

        if((storageState == Environment.MEDIA_MOUNTED) || storageState == Environment.MEDIA_MOUNTED_READ_ONLY) {
            val homeAppContext = createPackageContext("uk.ac.swansea.dascalu.dvmicc.home", Context.CONTEXT_IGNORE_SECURITY)
            ...
            if (settingsFile.exists()) {
                ...
                Toast.makeText(this, "Security level settings file is not present!",
                        Toast.LENGTH_LONG).show()

                return getString(R.string.lowSecurityLevel)
            }
        } else {
            Toast.makeText(this, "External storage is not present!",
                    Toast.LENGTH_LONG).show()
